...
static bool is_last_line(unsigned int hour, unsigned int type) {
  return  hour == HOURS  && type == TYPE - 1u;
}

void array_dump(DelayTable a) {
  for (unsigned int type = 0u; type < TYPE; ++type) {
    for (unsigned int hour = 1u; hour <= HOURS; ++hour) {
...
void array_from_file(DelayTable array, const char *filepath) {
  ...
  while (!feof(file) && i<=HOURS) 
  {
    ...
    array[last_mile_info.type][last_mile_info.hour]=last_mile_info;
    array[layover_info.type][layover_info.hour]=layover_info;
